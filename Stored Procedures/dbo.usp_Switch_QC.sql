CREATE PROCEDURE [dbo].[usp_Switch_QC]
AS
/*
-------------------------------------------------------------------------------
Author			Jeremy Fifer
...
DECLARE @flowName AS VARCHAR(20) = 'QC'
...
EXEC ProcessStatus_Update 'QC Switch SP', @lastRunDate;
...
WHERE b.textValue LIKE '%.pdf'
AND b.deleteX <> 'yes'
AND b.optionCaption in ('CanvasHiResFront UNC File')	--added canvas - iFrame
AND (SUBSTRING(a.productCode, 1, 2) = 'FB') 
...
SET @json = (SELECT orderid,@flowName as switchflow from tblSwitch_QC FOR JSON PATH);
