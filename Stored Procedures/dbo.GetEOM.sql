...
10/13/21		JF, fixing tax, fixing monthly billing shenanigans (e.g., 1234-MRK1234567
05/02/22		Bobby, added not exists code from this https://app.clickup.com/t/2u3yv46
-------------------------------------------------------------------------------------
*/
...
CASE
	WHEN LEN(z.customData) = 9 AND (RIGHT(z.customData, 10) LIKE 'HOM%' OR RIGHT(z.customData, 10) LIKE 'MRK%') THEN ' '
	ELSE z.customData
END AS 'customData',
a.orderID, z.rowID,
'1' AS 'source',
...
INNER JOIN tblChaseTransactions z ON a.orderNo = RIGHT(REPLACE(z.orderNo, '000000', ''), 10)
...
AND NOT EXISTS
	(SELECT TOP 1 1
	FROM tblChaseTransactions z WHERE
	a.orderNo = RIGHT(REPLACE(z.orderNo, '000000', ''), 10))
GROUP BY
...
INNER JOIN tblOrders x ON o.ID = x.orderID - @OrderOffset
...
--///// update acutal tax rate
IF OBJECT_ID('tempdb..#temp') IS NOT NULL 
DROP TABLE #temp

SELECT orderNo,shipping_street,shipping_suburb, shipping_zip, 
CAST(NULL AS VARCHAR(10)) AS TaxRate,
CAST(NULL AS VARCHAR(50)) AS taxcity 
INTO #temp 
FROM tblEOM_Bouncer 
WHERE shipping_state = 'ca'

--CKB (jf replaced this code with what follows below)
						--declare 
						--  @orderNo as varchar(20)
						--, @address as varchar(100)
						--, @shipping_city as varchar(50)
						--, @shipping_zip as varchar(5)
						--, @taxrate varchar(20)
						--, @taxcity varchar(50)


						--declare rCursor cursor for
						--	select orderNo,shipping_street,shipping_suburb, shipping_zip from #temp 
						--open rCursor
						--fetch next from rCursor into @orderNo , @address , @shipping_city, @shipping_zip

						--while @@Fetch_status = 0
						--begin

						--	exec GetAddressTaxRate @address, @shipping_city, @shipping_zip, @taxrate OUTPUT, @taxcity OUTPUT

						--	update #temp set taxrate = @taxrate, taxcity = @taxcity
...
-- JF:
IF OBJECT_ID('tempdb..#tempTAXI') IS NOT NULL 
DROP TABLE #tempTAXI
...
INSERT INTO #tempTAXI (OrderNo, shipAddress, shipCity, shipZip)
SELECT orderNo, shipping_street, shipping_suburb, shipping_zip 
FROM #temp 
...
BEGIN
	...
	SELECT  @orderNo = ISNULL(orderNo, 0),
			@address = ISNULL(shipAddress, 0),
		    @shipping_city = ISNULL(shipCity, 0),
			@shipping_zip = ISNULL(shipZip, 0)
	FROM #tempTAXI
...
UPDATE e 
SET taxRate = t.taxRate, 
	taxCity = t.taxCity
FROM #temp t
INNER JOIN tbleom_bouncer e ON t.orderNo = e.orderNo
...
taxRate as AddressTaxRate, taxCity as AddressTaxCity
FROM tblEOM_Bouncer
...
SELECT '?', 'Yes', 
CONVERT(SMALLDATETIME, z.transactionDateTime) AS 'transactionDateTime', 
RIGHT(REPLACE(z.orderNo, '000000', ''), 10), 
'', '', 
'','','','', 
