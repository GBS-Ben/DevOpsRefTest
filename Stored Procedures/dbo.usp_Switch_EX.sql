CREATE proc [dbo].[usp_Switch_EX]
AS
/*
-------------------------------------------------------------------------------
Author      Jeremy Fifer
Created     10/20/16
Purpose     Pulls EX products into Switch for production.
------------------------------------------------------------------------------
...
12/20/19		JF, CRUD.
12/29/19		BS, GOOD TO GO file name change logic
...
DECLARE @flowName AS VARCHAR(20) = 'EX'

DECLARE @lastRunDate datetime = getdate();
EXEC ProcessStatus_Update 'EX Switch SP', @lastRunDate;
...
TRUNCATE TABLE tblSwitch_EX
INSERT INTO tblSwitch_EX (orderID, orderNo, orderDate, customerID, 
...
WHERE
DATEDIFF(MI, a.created_on, GETDATE()) > 60 --increased the wait time from 10 minutes
...
AND SUBSTRING(p.productCode, 3, 2) = 'EX'
...
--// confirm that non-sport EXs have variableTopName that are blank.
UPDATE tblSwitch_EX
SET variableTopName = ''
...
SET @json = (SELECT orderid,@flowName as switchflow from tblSwitch_EX FOR JSON PATH);
...
IF OBJECT_ID('tempdb..#tempResubChoiceEX') IS NOT NULL 
DROP TABLE #tempResubChoiceEX

CREATE TABLE #tempResubChoiceEX (
...
DECLARE @NumberRecords_rs INT, 
				 @RowCount_rs INT,
				 @OPID_rs INT,
				 @MostRecent_ResubQTY_rs INT

INSERT INTO #tempResubChoiceEX (OPID)
...
BEGIN
	 SELECT @OPID_rs = OPID
	 FROM #tempResubChoiceEX
	 WHERE RowID = @RowCount_rs

	 SET @MostRecent_ResubQTY_rs = (SELECT TOP 1 resubmitQTY
															FROM tblSwitch_resubOption
															WHERE OPID = @OPID_rs
															ORDER BY resubmitDate DESC)
	
	UPDATE tblSwitch_EX
	SET shipsWith = 'RESUB ' + CONVERT(VARCHAR(50), ISNULL(@MostRecent_ResubQTY_rs, 1))
...
TRUNCATE TABLE tblSwitch_EX_distinctIDCount
INSERT INTO tblSwitch_EX_distinctIDCount (orderID, ordersProductsID)
SELECT DISTINCT orderID, ordersProductsID
FROM tblSwitch_EX

TRUNCATE TABLE tblSwitch_EX_distinctIDCount2
INSERT INTO tblSwitch_EX_distinctIDCount2 (orderID, countOrderID)
SELECT orderID, COUNT(orderID) AS 'countOrderID'
FROM tblSwitch_EX_distinctIDCount
GROUP BY orderID
ORDER BY orderID
...
FROM tblSwitch_EX a 
INNER JOIN tblSwitch_EX_distinctIDCount2 b
...
IF OBJECT_ID(N'tblSwitch_EX_displayCount', N'U') IS NOT NULL
DROP TABLE tblSwitch_EX_displayCount

CREATE TABLE tblSwitch_EX_displayCount 
	(RowID INT IDENTITY(1, 1), 
	orderID INT, 
...
TRUNCATE TABLE tblSwitch_EX_displayCount
INSERT INTO tblSwitch_EX_displayCount (orderID, ordersProductsID, totalCount)
...
BEGIN
	...
	FROM tblSwitch_EX_displayCount
...
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '1 of', '01 of') WHERE displayCount LIKE '1 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '2 of', '02 of') WHERE displayCount LIKE '2 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '3 of', '03 of') WHERE displayCount LIKE '3 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '4 of', '04 of') WHERE displayCount LIKE '4 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '5 of', '05 of') WHERE displayCount LIKE '5 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '6 of', '06 of') WHERE displayCount LIKE '6 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '7 of', '07 of') WHERE displayCount LIKE '7 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '8 of', '08 of') WHERE displayCount LIKE '8 of%'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, '9 of', '09 of') WHERE displayCount LIKE '9 of%'

UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 1', 'of 01') WHERE displayCount LIKE '%of 1'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 2', 'of 02') WHERE displayCount LIKE '%of 2'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 3', 'of 03') WHERE displayCount LIKE '%of 3'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 4', 'of 04') WHERE displayCount LIKE '%of 4'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 5', 'of 05') WHERE displayCount LIKE '%of 5'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 6', 'of 06') WHERE displayCount LIKE '%of 6'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 7', 'of 07') WHERE displayCount LIKE '%of 7'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 8', 'of 08') WHERE displayCount LIKE '%of 8'
UPDATE tblSwitch_EX SET displayCount = REPLACE(displayCount, 'of 9', 'of 09') WHERE displayCount LIKE '%of 9'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 1 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 2 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 3 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 4 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 5 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
INNER JOIN tblOrdersProducts_ProductOptions b
	ON a.ordersProductsID = b.ordersProductsID
WHERE  b.optionCaption = 'Team 6 File Name'				-- only used for FBQX so I don't believe applies here iframe
AND b.deleteX <> 'yes'
...
--// Set flags
UPDATE tblSwitch_EX SET switch_approve = 0
UPDATE tblSwitch_EX SET switch_print = 0
UPDATE tblSwitch_EX SET switch_approveDate = GETDATE()
UPDATE tblSwitch_EX SET switch_printDate = GETDATE()
UPDATE tblSwitch_EX SET switch_createDate = GETDATE()
...
/* 
GNEX00-019	Babysitter Checklist
GNEX00-020	Emergency First Aid
GNEX00-021	Emergency Phone Numbers
GNEX00-022	Important Phone Numbers
GNEX00-023	Numeros de Emergencia
GNEX00-025	Primeros Auxilios
*/

UPDATE tblSwitch_EX
SET UV = 0
WHERE productCode = 'GNEX00-019'
OR productCode = 'GNEX00-020'
OR productCode = 'GNEX00-021'
OR productCode = 'GNEX00-022'
OR productCode = 'GNEX00-023'
OR productCode = 'GNEX00-025'
...
--INSERT INTO tblSwitch_EXLog (PKID, ordersProductsID, insertedOn)
--SELECT DISTINCT PKID, ordersProductsID, GETDATE()
--FROM tblSwitch_EX_ForOutput
...
,jsonData = 
       (select *
       from tblSwitch_EX_ForOutput b
       where a.PKID = b.PKID
       for json path)
from tblSwitch_EX_ForOutput a
...
SET switch_create = 1,
	...
	fastTrak_resubmit = 0
FROM tblOrders_Products op
INNER JOIN tblSwitch_EX_ForOutput t ON op.ID = t.ordersProductsID
