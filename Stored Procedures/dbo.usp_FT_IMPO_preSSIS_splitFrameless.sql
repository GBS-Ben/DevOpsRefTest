CREATE PROC [dbo].[usp_FT_IMPO_preSSIS_splitFrameless]
AS
/*
-------------------------------------------------------------------------------
-- Author      Jeremy Fifer
...
--03/23/21		BS, removed IntranetPDF oppo
-------------------------------------------------------------------------------
*/
SET XACT_ABORT ON
...
AND op.productCode NOT IN ('NB2CRB-001-100', 'NB2COB-001-100')
AND x.deleteX <> 'yes'
AND x.optionCaption IN ( 'CanvasHiResFront File Name') --bjs iframe conversion --this indicates CANVAS (it is exclusive to CANVAS OPIDs) as well as supplies the row needed for the image path.
AND op.ID NOT IN
	(SELECT ordersproductsID
