...
CREATE PROC [dbo].[usp_FT_Badges_pSlips_iFrame_02052021]
...
AND p.ID NOT IN
	...
	WHERE deleteX <> 'yes'
	AND optionCaption = 'Intranet PDF') -- THIS IS CANVAS, We don't want that in the oldschool insert.
...
WHERE p.deleteX <> 'yes'
AND z.deleteX <> 'yes'
AND (z.optionCaption = 'Intranet PDF'AND z.optionID = 539) --THIS IS CANVAS (it is exclusive to CANVAS OPIDs) as well as supplies the row needed for the image path.
--AND p.fastTrak_preventLabel = 0 --pulled these two checks out. If the OPID is in one of the tables below, it qualifies for a label.
AND p.fastTrak_shippingLabelOption1 <> 1
AND p.ID IN
	 (SELECT ordersproductsID 
...
--This commented out section was the code that previous 5 update statements have replaced, 09/25/19, jf.
								...
								--UPDATE tblFT_Badges_pSlips
								--SET orderType = 'fasTrak'
								--WHERE orderType <> 'Custom' --select * from tblorders where orderno = 'hom916684'
								--AND orderNo IN
								--	(SELECT DISTINCT orderNo
								--	FROM tblOrders 
								--	WHERE orderID IN
