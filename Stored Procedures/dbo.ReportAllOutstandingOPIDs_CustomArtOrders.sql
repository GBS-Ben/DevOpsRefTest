CREATE PROCEDURE [dbo].[ReportAllOutstandingOPIDs_CustomArtOrders] 
AS

IF OBJECT_ID('tempdb..#Dubnobass') IS NOT NULL DROP TABLE #Dubnobass
CREATE TABLE #Dubnobass(
...
INSERT INTO #Dubnobass ([Order No], [Order Status], [Order Type], [Payment Status], [Local Pickup], [Date], [OPID], [OPID Status], [Product Code], [Product Name], [Type], [Days], [Mailing], [FT Status LastMod])
...
AND o.orderStatus IN ('On Proof', 'In Art')
AND DATEDIFF(MM, o.orderDate, GETDATE()) < 8
