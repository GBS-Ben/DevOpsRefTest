CREATE procEDURE[dbo].[usp_expediteFlag_OPID] @OPID INT = 0
AS
/*
-------------------------------------------------------------------------------
Author      Jeremy Fifer
...
SET @currentExpressProductionStatus = (SELECT ISNULL(COUNT(oppo.PKID), 0)
									...
									AND (oppo.optionID = 490
									 OR (optionCaption = 'Express Production' AND (textValue LIKE 'Yes%' OR textValue LIKE 'Express%' OR ISNULL(textValue,'') = ''))	-- added textValue qualifier for iFrame conversion
										)
...
BEGIN
	...
	SELECT @OPID, 252, 'Express Production', 0, 'Description', 1, 'Yes', 0
