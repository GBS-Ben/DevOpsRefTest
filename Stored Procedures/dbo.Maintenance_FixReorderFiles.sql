CREATE PROCEDURE [dbo].[Maintenance_FixReorderFiles]
...
SET NOCOUNT ON;
		...
		BEGIN
			...
			INNER JOIN [tblOrders] o ON o.[orderID] = op.[orderID]
				AND orderStatus NOT IN ('Failed','Cancelled','Delivered','MIGZ')
			WHERE  o.[orderDate] BETWEEN '2021-03-05 20:00:00.000' 
				AND DATEADD(mi,-10,GETDATE()) --give the order time to arrive
				AND LEFT([productCode], 2) IN ('BP','SN','NP','EV','CM','NB', 'FB', 'BB','CA','NC','PN', 'FA')--,'','','','','',''
				...
				AND (
					...
					EXISTS (SELECT TOP 1 1  FROM tblOrdersProducts_ProductOptions pf WHERE pf.ordersProductsID = op.Id and optionCaption = 'Reorder')
					AND 
		...
		WHERE oppo.[OptionCaption] IN ( 'File Name 2')
		...
		INSERT #NewOppos( [OPID],[newOptionCaption], [textValue])
		SELECT DISTINCT  oppo.[ordersProductsID] AS [OPID], 'CanvasHiResFront' AS [newOptionCaption], 
				CASE WHEN REPLACE(REPLACE(REPLACE(REPLACE(textValue,'canvasa','canvaslb'),'rembrandt','canvaslb'),'canvas5b','canvaslb'),'houseofmagnets','markful') LIKE 'HTTP%' THEN REPLACE(REPLACE(REPLACE(REPLACE(textValue,'canvasa','canvaslb'),'rembrandt','canvaslb'),'canvas5b','canvaslb'),'houseofmagnets','markful')
				ELSE 'https:' + REPLACE(REPLACE(REPLACE(REPLACE(textValue,'canvasa','canvaslb'),'rembrandt','canvaslb'),'canvas5b','canvaslb'),'houseofmagnets','markful')
		...
		INSERT #NewOppos( [OPID],[newOptionCaption], [textValue])
		SELECT DISTINCT  ordersProductsID AS [OPID], 'CanvasHiResBack' AS [newOptionCaption], TextValue AS [textValue] 
		FROM [tblOrdersProducts_ProductOptions] oppo
		INNER JOIN #CandidateOpids c ON c.[OPID] = oppo.[ordersProductsID]
		INNER JOIN [tblProductOptions] [po] ON [po].[OptionCaption] = oppo.[OptionCaption] 
		...
		SELECT DISTINCT  oppo.[ordersProductsID] AS [OPID], 'CanvasHiResBack' AS [newOptionCaption], 
				CASE WHEN [textValue] LIKE 'HTTP%' THEN REPLACE(REPLACE(REPLACE(textValue,'canvasa','canvaslb'),'rembrandt','canvaslb'),'houseofmagnets','markful')
				ELSE 'https:' + REPLACE(REPLACE(REPLACE(textValue,'canvasa','canvaslb'),'rembrandt','canvaslb'),'houseofmagnets','markful')
				END AS [textValue] 
		FROM [tblOrdersProducts_ProductOptions] oppo
		INNER JOIN #CandidateOpids c ON c.[OPID] = oppo.[ordersProductsID]
		INNER JOIN [tblProductOptions] [po] ON [po].[OptionCaption] = oppo.[OptionCaption] 
...
---------------------------------------------------------------------------------------------------
	...
	---add missing back unc file
	 INSERT tblordersproducts_productoptions(
	ordersProductsID	,optionID,	optionCaption,	optionPrice	,optionGroupCaption,	textValue,	deletex	,optionQty,	ordersProductsGUID,	created_on,	modified_on)
	SELECT op.Id,764,	'CanvasHiResBack UNC File'	,0.00	,'Custom Info'	,@UncBasePath + [dbo].[fn_GetOppoFileName](op.Id, 2,'pdf'),   	0,	0,	op.ordersProductsGUID,	getdate(), getdate()
		   FROM tblordersProducts_ProductOptions oppo
		   INNER JOIN tblOrders_Products op ON op.Id = oppo.ordersProductsID
		   INNER JOIN tblOrders o ON o.OrderID = op.orderID 
		   WHERE orderStatus NOT IN ('cancelled', 'delivered', 'failed')
