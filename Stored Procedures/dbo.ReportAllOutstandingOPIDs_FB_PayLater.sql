CREATE PROCEDURE [dbo].[ReportAllOutstandingOPIDs_FB_PayLater] 
AS

IF OBJECT_ID('tempdb..#Interzone') IS NOT NULL DROP TABLE #Interzone
...
INSERT INTO #Interzone ([Order No], [Order Status], [Order Type],[Order Total], [Payment Method], [Payment Status], [Local Pickup], [Date], [OPID], [OPID Status], [Product Code], [Product Name], [Type], [Days], [Mailing], [FT Status LastMod])
...
o.orderTotal,
o.paymentMethod, o.displayPaymentStatus AS 'Payment Status',
...
SELECT [Order No], [Order Status], [Order Type], [Order Total], [Payment Method], [Payment Status], [Local Pickup], [Mailing], [Date], [OPID], [OPID Status], [Product Code], [Product Name], [Type], [Days], [FT Status LastMod]
FROM #Interzone
ORDER BY [Date] DESC