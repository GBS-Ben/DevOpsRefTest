CREATE  proc [dbo].[GBSReports_Marketing_FB_Last7Days] as
...
,op.productname
,case when CHARINDEX('(#',op.productName) > 0 then  substring(op.productName,1,CHARINDEX('(#',op.productName) - 1) else op.productName end ProductName
,c.email
,FirstName = coalesce(
	case 
...
where o.orderStatus not in ('Cancelled','Failed')
	and op.deletex <> 'yes'
	and left(op.productCode,2) = 'FB'
	and o.orderDate >= getdate() - 7