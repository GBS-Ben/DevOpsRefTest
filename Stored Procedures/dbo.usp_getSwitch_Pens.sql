...
CREATE PROCEDURE [dbo].[usp_getSwitch_Pens] 
AS


DECLARE @lastRunDate datetime = getdate();
EXEC ProcessStatus_Update 'PEN-S Switch', @lastRunDate;
-------------------------------------------------------------------------------
-- Author      Jeremy Fifer
-- Created     08/3/15
...
--10/12/2020	BJS Goldstar pens work
--02/03/2021	BJS iframe conversion
--02/09/2021	BJS, added credit payment status
--02/28/2022	BJS, updated imprint color
-------------------------------------------------------------------------------
...
z.shipping_Suburb AS 'City', z.shipping_State AS 'State', z.shipping_PostCode AS 'Zip',
	...
	ISNULL((SELECT TOP 1 oppo.textValue
	FROM tblOrdersProducts_productOptions oppo 
	WHERE op.ID = oppo.ordersProductsID 
		AND oppo.OptionCaption = 'Imprint Color'
		AND oppo.deletex <> 'yes'
		),ISNULL(v.vImprintColor,'White')) AS vImprintColor, --Imprint Color  --when null then WHITE
	...
	FROM tblOrdersProducts_productOptions oppo 
	WHERE op.ID = oppo.ordersProductsID 
		AND oppo.OptionCaption IN ( 'Intranet PDF', 'CanvasHiResFront File Name')
		AND oppo.deletex <> 'yes'
		)  AS 'PDF',    --- Intranet PDF  
		op.Id	--SELECT * 
FROM tblOrders a
...
WHERE
	...
	AND v.[Name] = 'Goldstar'  
	AND DATEDIFF(mi,a.orderDate,GETDATE()) > 60 
	AND (
		--3.a
