CREATE PROC [dbo].[usp_FT_getActive_Sort_ALT]	
...
--
-- 8/26/16		Added fasTrak related code to include First Class products (FC).
-- 8/9/17		JF, added JU/EX/BU to FT code.
-------------------------------------------------------------------------------
...
AND op.deleteX <> 'Yes'
AND a.orderStatus NOT IN ('Failed', 'Cancelled', 'Delivered', 'MIGZ', 'In Transit', 'In Transit USPS', 'Waiting For Payment', 'Waiting On Customer', 'Waiting For New Art', 'GTG-Waiting For Payment')
--AND p.fastTrak_productType IN ('Name Badge', 'QuickCard', 'QM', 'CM', 'CACX', 'Pen', 'FC', 'NC', 'JU', 'EX', 'BU')
