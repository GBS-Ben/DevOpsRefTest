CREATE proc [dbo].[usp_GetOppos_EditProduct_iFrame_02052021] 
...
where op.id = @ordersProductsId
	...
	and po.isEditable = 1
	and pog.isActive = 1
group by ppo.optionId,pog.optionGroupCaption, ppo.optionGroupId,pog.optionGroupType, po.optionCaption,case when oppo.optionID is not null then 1 else 0 end,po.isActive,rtrim(substring(isnull(oppo.textValue,''),1,case when charindex('[',isnull(oppo.textValue,'')) > 0 then charindex('[',isnull(oppo.textValue,''))-1 else len(isnull(oppo.textValue,'')) end))
