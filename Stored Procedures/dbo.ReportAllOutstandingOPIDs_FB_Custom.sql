CREATE PROCEDURE [dbo].[ReportAllOutstandingOPIDs_FB_Custom] 
AS

IF OBJECT_ID('tempdb..#Passover') IS NOT NULL DROP TABLE #Passover
...
INSERT INTO #Passover ([Order No], [Order Status], [Order Type], [Payment Method], [Payment Status], [Local Pickup], [Date], [OPID], [OPID Status], [Product Code], [Product Name], [Type], [Days], [Mailing], [FT Status LastMod])
