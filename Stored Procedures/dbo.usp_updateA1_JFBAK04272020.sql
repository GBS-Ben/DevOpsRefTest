CREATE PROC [dbo].[usp_updateA1_JFBAK04272020]
AS
/*
-------------------------------------------------------------------------------------
 Author      Jeremy Fifer
...
BEGIN TRY
	...
	*/
	...
	/* ** The following code was rolled back on 10/30 because there were less A1 tickets being created. How? I have no idea. The original code that it replaced, is below this block, and when time permits, do a A/B on this and find the differences.

	UPDATE tblOrders
	SET a1 = 0
	WHERE a1 = 1
	AND orderStatus NOT IN ('failed', 'cancelled', 'delivered', 'On HOM Dock', 'In Transit', 'In Transit USPS', 'MIGZ')

	UPDATE a
	SET a1 = 1 --select count(orderid)
	FROM tblOrders a
	INNER JOIN tblCustomers c ON a.customerID = c.customerID
	INNER JOIN tblCustomers_ShippingAddress s ON a.orderNo = s.orderNo
	WHERE a1 = 0 
	AND	a.orderStatus NOT IN ('failed', 'cancelled', 'delivered', 'On HOM Dock', 'In Transit', 'In Transit USPS', 'MIGZ')
	AND a.orderJustPrinted = 0 --orders which have not already had an A1 label generated. this field is updated by label creation.
	AND a.displayPaymentStatus = 'Good'
	AND a.orderType <> 'Custom'
	AND a.A1_expediteShipFlag <> 1 --non-expedited order; next three conditions cover the bulk of expedited shippingDescs:
	AND a.shippingDesc NOT LIKE '%Air%' 
	AND a.shippingDesc NOT LIKE '%Next%' 
	AND a.shippingDesc NOT LIKE '%Express%' 
	AND s.rdi = 'R' --residential
	AND s.isValidated = 1 --address has been validated
	AND a.orderID IN --orders that have qualifying A1 products within, at a product quantity of "1" for numUnits * productQuantity; can only have a SUM of "1" per order in this criteria.
		(SELECT orderID
		FROM tblOrders_Products op
		INNER JOIN tblProducts pr ON op.productID = pr.productID
		WHERE op.deleteX <> 'yes'
		AND ((pr.productType = 'Stock' AND pr.productName LIKE '%pad%' AND pr.productName LIKE '%calendar%') --Stock Cal Pad Mags
				OR (pr.productType = 'Stock' AND pr.productName LIKE '%quickStix%' AND pr.productName NOT LIKE '%create%') --Stock QuickStix
				OR (pr.productType = 'Stock' AND SUBSTRING(pr.productCode, 3, 2) = 'QS' AND pr.productName NOT LIKE '%create%') --Stock QuickStix
				OR (pr.productType = 'Stock' AND SUBSTRING(pr.productCode, 1, 2) = 'QS' AND pr.productName NOT LIKE '%create%') --Stock QuickStix
				OR (pr.productType = 'Stock' AND pr.productName LIKE 'Note Card Set%')) --Note Card Set (updated 2/17/16, jf; as per KH: "A1 tickets should include a single order of a 72 note card pak for the padded envelope rate")
		GROUP BY orderID
		HAVING SUM(op.productQuantity * pr.numUnits) = 1)
	AND a.orderID NOT IN --orders that do not contain non-A1 products as shown above.
		(SELECT orderID
		FROM tblOrders_Products prs
		WHERE deleteX <> 'yes'
		AND productID NOT IN
				(SELECT productID 
				FROM tblProducts
				WHERE (prs.productType = 'Stock' AND prs.productName LIKE '%pad%' AND prs.productName LIKE '%calendar%') --Stock Cal Pad Mags
				OR (prs.productType = 'Stock' AND prs.productName LIKE '%quickStix%' AND prs.productName NOT LIKE '%create%') --Stock QuickStix
				--OR (prs.productType = 'Stock' AND SUBSTRING(prs.productCode, 3, 2) = 'QS' AND prs.productName NOT LIKE '%create%') --Stock QuickStix
				--OR (prs.productType = 'Stock' AND SUBSTRING(prs.productCode, 1, 2) = 'QS' AND prs.productName NOT LIKE '%create%') --Stock QuickStix
				--OR (prs.productType = 'Stock' AND prs.productName LIKE 'Note Card Set%') --these three commented-out sections are not in original, (but seem needed).
				)
		)
	AND a.orderID NOT IN -- orders that do not have the "Production Timing: Complete Schedule (ship later)" product option selected.
		(SELECT orderID
		FROM tblOrders_Products opx
		INNER JOIN tblOrdersProducts_productOptions oppo ON opx.ID = oppo.ordersProductsID
		WHERE opx.deleteX <> 'yes'
		AND oppo.deleteX <> 'yes'
		AND oppo.optionCaption = 'Complete Schedule (ship later)')
