...
-- Description:	Get NCC orders to send UPS shipped email
-- =============================================
CREATE PROCEDURE [dbo].[Email_Shipped_UPS_NCC] 
AS
BEGIN
	...
	BEGIN TRY
		...
		BEGIN TRANSACTION
			...
			SELECT DISTINCT
			c.email,o.orderNo,jt.trackingNumber, 'email_shipped_ncc','NCC',
				'{  
					"identifyBy": "email",
					 "to":{  
						  "contact":{  
			...
			INNER JOIN tblCustomers c ON o.customerID = c.customerID
			INNER JOIN tblCustomers_ShippingAddress cs ON o.orderNo = cs.orderNo
			where jt.trackingnumber like '1z%'
